...
def st_gallery(with_expander=True):
    gallery = Gallery
    if with_expander:
...
def st_interactive_chart(with_expander=True):
    if with_expander:
...
def st_gallery_code():
    ...
    with st.expander("Gallery Code"):
        # Gallery object
        st.write(":blue[Gallery Object example:]")
        st.code(gallery_obj, language="python", line_numbers=True)
        if st.button("copy gallery object"):
            clipboard.copy(gallery_obj)

        st_vertical_space(2)

        # Gallery class
        st.write(":blue[Gallery Class:]")
        st.code(gallery_cls, language="python", line_numbers=True)

        if st.button("copy gallery class"):
            clipboard.copy(gallery_obj)


def st_interactive_chart_code():
    st_interactive_chart_txt = '''
    def st_interactive_chart(with_expander=True):
        if with_expander:
            with st.expander("Interactive Chart"):
                interactive_chart()
    ...
    with st.expander("Interactive Chart Code"):
        st.write(":blue[st_interactive_chart(with_expander=True):]")
        st.code(st_interactive_chart_txt, language="python", line_numbers=True)
        if st.button("copy st_interactive_chart "):
            clipboard.copy(st_interactive_chart_txt)
        st_vertical_space(2)
        st.write(":blue[interactive_chart(downloadable=True):]")
        st.code(interactive_chart_txt, language="python", line_numbers=True)
        if st.button("copy interactive_chart"):
            clipboard.copy(interactive_chart_txt)
        st_vertical_space(2)
        st.write(":blue[get_chart(x, y, plt_type, chart_type):]")
        st.code(get_chart_txt, language="python", line_numbers=True)
        if st.button("copy get_chart"):
            clipboard.copy(get_chart_txt)


def example_generic_inputs_code():
    generic_inputs_text = '''
    def example_generic_inputs():
        with st.expander("Generic Inputs"):
            # Single value example
    ...
    with st.expander("Generic Inputs Code"):
        st.write(":blue[example_generic_inputs():]")
        st.code(generic_inputs_text, language="python", line_numbers=True)
        if st.button("copy generic_inputs"):
            clipboard.copy(generic_inputs_text)
        st_vertical_space(2)

        st.write(":blue[generate_inputs():]")
        st.code(generate_inputs_text, language="python", line_numbers=True)
        if st.button("copy generate_inputs_text"):
            clipboard.copy(generate_inputs_text)

def main():
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("Examples")
        st_gallery(with_expander=True)
        st_interactive_chart(with_expander=True)
        example_generic_inputs()
    with c2:
        with st.columns([12, 2])[1]:
            st.subheader("Codes")
        st_gallery_code()
        st_interactive_chart_code()
        example_generic_inputs_code()
